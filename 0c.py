...
class NetworkScanner(QThread):
    ...
    def arp_scan_multiple(self, network, timeout=3, retry=3):
        """
        Melakukan ARP scan berkali-kali untuk mendapatkan lebih banyak device
        
        Args:
            network: Network CIDR (e.g., "192.168.1.0/24")
            timeout: Timeout per scan dalam detik
            retry: Jumlah pengulangan scan
        
        Returns:
            Set of tuples (ip, mac)
        """
        devices_found = set()
        
        for attempt in range(retry):
            self.progress_update.emit(f"Scanning attempt {attempt + 1}/{retry}...")
            ...
            try:
                ...
                result = srp(packet, timeout=timeout, verbose=0, inter=0.1)[0]
                
                for sent, received in result:
                    devices_found.add((received.psrc, received.hwsrc))
                
                self.progress_update.emit(f"Found {len(devices_found)} devices so far...")
                
            except Exception as e:
                self.progress_update.emit(f"Error in attempt {attempt + 1}: {e}")
    ...
    def icmp_ping_sweep(self, network):
        """
        Melakukan ICMP ping sweep untuk 'membangunkan' device
        Ini membantu device untuk merespons ARP request
        """
        self.progress_update.emit("Performing ICMP ping sweep...")
        ...
        try:
            ...
            # Buat list IP untuk di-ping
            ip_list = [str(ip) for ip in net.hosts()]
            
            # Batasi jika terlalu banyak (misal max 254 untuk /24)
            if len(ip_list) > 254:
                ip_list = ip_list[:254]
            
            # Ping menggunakan ICMP dengan scapy
            icmp = IP(dst=ip_list)/ICMP()
            
            # Send tanpa menunggu reply (kita hanya ingin memicu device)
            send(icmp, verbose=0, iface=get_default_iface().name)
            
            # Tunggu sebentar agar device sempat merespons
            time.sleep(1)
            
        except Exception as e:
            self.progress_update.emit(f"ICMP sweep error: {e}")
    
    def check_arp_table(self):
        """
        Membaca ARP table sistem untuk mendapatkan device tambahan
        """
        devices = set()
        ...
        try:
            ...
            for line in result.stdout.splitlines():
                ...
                if len(parts) >= 5 and parts[0] != '':
                    # Format: IP dev IFACE lladdr MAC REACHABLE/STALE/...
                    ip = parts[0]
                    if 'lladdr' in parts:
            ...
            self.progress_update.emit(f"Found {len(devices)} devices in ARP table")
            
        except Exception as e:
            self.progress_update.emit(f"Error reading ARP table: {e}")
        
        return devices
    
    def run(self):
        """Menjalankan scan jaringan dengan metode gabungan"""
        ...
        self.progress_update.emit(f"Network: {self.network}, Gateway: {self.gateway}")
        
        all_devices = set()
        
        # Metode 1: ICMP Ping Sweep (untuk membangunkan device)
        self.icmp_ping_sweep(self.network)
        
        # Metode 2: Multiple ARP Scans dengan timeout lebih lama
        arp_devices = self.arp_scan_multiple(
            network=self.network,
            timeout=5,  # Timeout lebih lama
            retry=3     # Scan 3 kali
        )
        all_devices.update(arp_devices)
        
        # Metode 3: Baca ARP table sistem
        arp_table_devices = self.check_arp_table()
        all_devices.update(arp_table_devices)
        
        # Convert ke list of Device objects
        devices = []
        for ip, mac in all_devices:
            is_gateway = (ip == self.gateway)
            is_self = (ip == self.my_ip or mac == self.my_mac)
        ...
        self.progress_update.emit(f"Scan selesai! Ditemukan {len(devices)} device")
